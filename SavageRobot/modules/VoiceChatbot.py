...
from SavageRobot import pbot as LYCIA
...
@LYCIA.on_message(filters.command("Savage"))
async def Lycia(_, message):
    if len(message.command) < 2:
        await message.reply_text("Lycia AI Voice Chatbot")
        return
    text = message.text.split(None, 1)[1]
    Savage = text.replace(" ", "%20")
    m = await message.reply_text("Savage Is Best...")
    try:
        L = await fetch(f"https://api.affiliateplus.xyz/api/chatbot?message={lycia}&botname=Savage&ownername=cyberking&user=1")
        chatbot = L["message"]
        ...
        await m.edit(str(e))
        return
    await m.edit("Made By @itz_cyberking_xd...")
